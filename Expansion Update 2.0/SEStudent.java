public class SEStudent extends Student {
  ...
  public void groupWork(Character enemy) throws Exception {
    if (this.currentKP == this.maxKP) {
      ...
      System.out.println(this.getName() + "has used GroupWork on " + enemy.getName() + "seriously damaging him");
      if (enemy.getHP() <= 0) {
        this.increaseEP(4);
      }
  ...
  public void groupDiscussion() throws Exception {
    if (this.currentKP == this.maxKP) {
      ...
      System.out.println(this.getName() + "has used groupDiscussion on " + this.getTeam().getName() + ", healing the members");
      this.increaseEP(4);
      this.currentKP = 0;
    } else throw new Exception("not enough KPS");
  }
